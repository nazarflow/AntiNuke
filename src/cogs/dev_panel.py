...
class CompareModal(disnake.ui.Modal):
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        ...
        try:
            if updates:
                await config_service.save_guild_roles(inter.guild.id, **updates)
...
class AddOwnerModal(disnake.ui.Modal):
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        ...
        if owner_id.isdigit():
            await admin_service.add_server_owner(inter.guild.id, int(owner_id))
...
class DelOwnerModal(disnake.ui.Modal):
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        owner_id = inter.text_values["owner_id"]
        if owner_id.isdigit():
            await admin_service.remove_server_owner(inter.guild.id, int(owner_id))
...
class FixSkipView(disnake.ui.View):
    ...
    async def btn_fix(self, button: disnake.ui.Button, inter: disnake.MessageInteraction):
        ...
        if self.missing_channels:
            guild_config = await config_service.get_guild_config(guild.id)
            if guild_config:
                cat_id, _ = guild_config
                ...
                if category:
                    ...
                    if created_channels:
                        await config_service.save_guild_log_channels(guild.id, **created_channels)
        ...
        if self.missing_roles:
            ...
            if created_roles:
                await config_service.save_guild_roles(guild.id, **created_roles)
...
class DevPanelView(disnake.ui.View):
    ...
    async def btn_doctor(self, button: disnake.ui.Button, inter: disnake.MessageInteraction):
        ...
        # Channels
        log_channels = await config_service.get_guild_log_channels(guild.id)
        channels_status = ""
        if log_channels:
            keys = [
            ...
            for key in keys:
                ch_id = getattr(log_channels, f"{key}_id", None)
                if ch_id:
        ...
        # Roles
        roles = await config_service.get_guild_roles(guild.id)
        roles_status = ""
        ...
        if roles:
            role_keys = ["quarantine", "quarantine_alt", "server_booster"]
            for key in role_keys:
                role_id = getattr(roles, f"{key}_id", None)
                if role_id:
        ...
        custom_roles = await admin_service.get_all_custom_roles(guild.id)
        if custom_roles:
            roles_status += "\n **Custom Admin Roles:** \n "
            for cr_id in custom_roles:
                cr = guild.get_role(cr_id)
                if cr:
                    limits = await admin_service.get_custom_role_limits(cr_id)
        ...
        # Custom Users
        custom_users = await admin_service.get_all_custom_users(guild.id)
        users_status = ""
        if custom_users:
            ...
            for cu_id in custom_users:
                ...
                if member:
                    limits = await admin_service.get_custom_user_limits(cu_id)
        ...
        # Owners
        owners = await admin_service.get_server_owners(guild.id)
